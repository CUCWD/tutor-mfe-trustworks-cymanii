...
tutor_hooks.Filters.IMAGES_BUILD.add_item(
    (
        ...
        "{{ MFE_DOCKER_IMAGE_TRUSTWORKS_CYMANII }}",
        (),
    )
)
...
tutor_hooks.Filters.CONFIG_DEFAULTS.add_items(
    [(f"MFE_{key}_TRUSTWORKS_CYMANII", value) for key, value in config.get("defaults", {}).items()]
)
tutor_hooks.Filters.CONFIG_UNIQUE.add_items(
    [(f"MFE_{key}_TRUSTWORKS_CYMANII", value) for key, value in config.get("unique", {}).items()]
)
